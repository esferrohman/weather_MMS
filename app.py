...
import numpy as np   # untuk validasi koordinat

# ---------- Konfigurasi halaman ----------
st.set_page_config(page_title="Dashboard Cuaca Tol Tangerang‑Merak",
                   layout="wide")
...
# ---------- URL data CSV ----------
SUMMARY_URL = ("https://docs.google.com/spreadsheets/d/e/"
               "2PACX-1vQF_6ZosMvgQQAAqDtKFXluP1Ad4wMnk4jYUVHQd6bc0NRRFBd4f4uc2euorAq98ua8uDP_1hls2AtN"
               "/pub?output=csv")
...
def load_summary(url: str) -> pd.DataFrame:
    ...
    if 'Update Terakhir (WIB)' in df.columns:
        df['Update Terakhir (WIB)'] = pd.to_datetime(
            df['Update Terakhir (WIB)'], errors='coerce')

    # Sanitasi angka umum
    numeric_cols = ['Temperatur (°C)', 'Kelembapan (%)',
                    'Kecepatan Angin (m/s)', 'Curah Hujan (mm)']
...
# Urutkan agar baris terbaru tiap lokasi paling atas
df_summary = df_summary.sort_values(['Lokasi', 'Update Terakhir (WIB)'],
...
with st.sidebar:
    ...
    # Tombol paksa refresh
    if st.button("🔄 Refresh Data Sekarang"):
        st.cache_data.clear()
        st.experimental_rerun()

    lokasi_order = ["Bitung", "Cikupa", "Balaraja Timur", "Balaraja Barat",
                    "Cikande", "Ciujung", "Serang Timur", "Serang Barat",
                    "Cilegon Timur", "Cilegon Barat", "Merak"]
...
# ---------- Kondisi cuaca semua lokasi ----------
st.subheader("📍 Kondisi Cuaca Terkini")
data_lain = (df_summary.sort_values('Update Terakhir (WIB)', ascending=False)
...
if lokasi_lain_urut:
    ...
    for idx, loc_name in enumerate(lokasi_lain_urut):
        ...
        bg = ("background-color:#ffe6e6; padding:4px; border-radius:8px;"
              if pd.notnull(curah) and curah > 0 else "")
        with cols[idx]:
            st.markdown(f"<div style='text-align:center; font-size:0.9em; {bg}'>{loc_name}</div>",
                        unsafe_allow_html=True)
...
if pd.notnull(data_terbaru['Update Terakhir (WIB)']):
    ...
    df_hist_lokasi_hariini = df_hist_lokasi[
        df_hist_lokasi['Update Terakhir (WIB)'].dt.date == tanggal_terbaru]
else:
    df_hist_lokasi_hariini = df_hist_lokasi.copy()

# ---------- Metrik detail lokasi terpilih ----------
...
with col2:
    ...
    if icon_code:
        st.markdown(
            ...
            f"<img src='http://openweathermap.org/img/wn/{icon_code}@4x.png' "
            f"style='width:100%; max-width:300px; margin-bottom:0.2em;'/>"
...
if isinstance(koor, str) and "," in koor:
    try:
        ...
        if np.isfinite(lat) and np.isfinite(lon):
            ...
            folium.Marker([lat, lon], tooltip=lokasi,
                          popup=popup_txt,
                          icon=folium.Icon(color="blue", icon="cloud")).add_to(m)
            st_folium(m, width=700, height=400)
            st.markdown("<div style='margin-top:-1em;'></div>",
                        unsafe_allow_html=True)
        else:
